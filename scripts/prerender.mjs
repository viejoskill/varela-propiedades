...
// Corregido contra el esquema real de la tabla `propiedades`.
//
// Produce:
...
//   /sitemap.xml
//
// NOTA: no consulta ni publica propietario_nombre / propietario_tel / notas.

import { writeFile, mkdir, rm } from 'node:fs/promises';
import { existsSync } from 'node:fs';

const SUPABASE_URL = 'https://fwwzjynerfghqsakkyvq.supabase.co';
const SUPABASE_KEY = 'sb_publishable_8F1HY-6hTbgfTnbKdCSAJA_J26tLIup';
const SITE = 'https://www.varelapropiedadescde.com.ar';
const WA_VENTAS = '5492246586030';

// Columnas publicas explicitas. NO usar select=*.
const COLS = [
  'id', 'codigo', 'titulo', 'tipo', 'operacion', 'estado', 'precio',
  'direccion', 'ciudad', 'provincia', 'ambientes', 'dormitorios', 'banios',
  'mts', 'mts_cubiertos', 'mts_semicubiertos', 'plantas', 'antiguedad',
  'amenities', 'descripcion', 'fotos', 'radar', 'destacada', 'created_at'
].join(',');
...
const res = await fetch(
  `${SUPABASE_URL}/rest/v1/propiedades?select=${COLS}&estado=in.(Disponible,Reservada,Reservado,Vendida,Vendido)&order=created_at.desc`,
